    ...
    public class PrefabBrush : GridBrush
    {
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position)
        {
            ...
            if (instance != null)
            {
                ...
                Vector3 cellPosition = new Vector3Int(position.x, position.y, m_Z);
                if (grid.cellLayout != GridLayout.CellLayout.Hexagon)
                    cellPosition += new Vector3(.5f, .5f, .5f);
                instance.transform.position = grid.LocalToWorld(grid.CellToLocalInterpolated(cellPosition));
            }
        }
